...
%% Environment Setup - Vertical corridor with parallel walls
...
%% CBF Parameters
d_safe = 0.3;           % Safety radius [m] (reduced from 0.4)
alpha_cbf = 0.5;        % CBF aggressiveness parameter (reduced from 1.5 - less conservative)
scan_downsample = 20;   % Use every Nth LiDAR beam (increased from 5 - fewer constraints)
constraint_range = 2.0; % Only use obstacles within this distance [m] (reduced from 3.0)
...
Q = diag([5, 10, 0.5]); % State tracking weights [x, y, theta] (reduced x and theta tracking)
R = diag([0.01, 0.5]);  % Control effort weights [v, omega] (allow more aggressive control)
...
%% Reference Trajectory - Navigate straight through corridor (through obstacle)
Tsim = 50;
t = 0:dt:Tsim;
